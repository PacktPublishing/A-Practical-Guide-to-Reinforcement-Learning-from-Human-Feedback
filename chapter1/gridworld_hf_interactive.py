...
class GridWorld:
    ...
    def q_learning_transfer(self, pre_learned_Q_values, epsilon=0.01):
        ...
        # Perform Q-learning in the new environment
        rewards_transfer, steps_per_episode, policy_progress = self.q_learning(epsilon=epsilon)
        return rewards_transfer, steps_per_episode, policy_progress
    ...
    def q_learning_with_human_interaction(self, num_episodes=10, learning_rate=0.1, discount_factor=0.9, epsilon=0.01, 
    ...
    def display_state_action(self, state, next_state):
        # Visualize the current state and action on a grid plot
        fig, ax = plt.subplots(figsize=(6, 6))

        # Display the current state and action for human evaluation
        ...
        # Clear the previous plot to avoid overlapping with the new plot
        plt.close()


    def get_human_feedback(self):
        # Simulate human feedback by allowing the user to input an evaluation
        # feedback = float(input("Please provide feedback for the action (0 for negative, 1 for neutral, 2 for positive): "))
        ...
        if feedback == 'y':
            ...
            return 0 # Adjust feedback to range from -1 to 1 for reward adjustment
...
rewards, steps_per_episode, policy_progress = grid_world.q_learning_with_human_interaction(num_episodes=2, 
                                                                                             learning_rate=0.1, 
...
rewards_transfer, steps_per_episode_transfer, policy_progress_transfer = grid_world.q_learning_transfer(grid_world.Q_values, epsilon=0.1)
grid_world.plot_policy_progress(policy_progress_transfer)
rewards_scratch, steps_per_episode, policy_progress = grid_world.q_learning_scratch()
grid_world.plot_policy_progress(policy_progress)
